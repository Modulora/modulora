...
function ToolReviewPage() {
  ...
  return (
    <div className="w-full max-w-7xl">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div><div className="mb-2 flex gap-2"><Badge variant="outline">Tool / site</Badge><Badge variant="secondary">Usefulness review</Badge></div><h1 className="text-2xl font-bold tracking-tight">{item.title}</h1><p className="mt-1 text-sm text-muted-foreground">@{item.namespace} · {item.siteDomain} · {item.category}</p></div><Button asChild variant="outline"><a href={item.siteUrl} target="_blank" rel="noreferrer"><External className="size-4" /> Open site</a></Button></div>
